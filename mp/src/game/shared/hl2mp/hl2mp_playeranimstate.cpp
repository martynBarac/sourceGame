...
void CHL2MPPlayerAnimState::Update( float eyeYaw, float eyePitch )
{
	...
	if ( SetupPoseParameters( pStudioHdr ) )
	{
		...
		// Pose parameter - Leaning left or right
		ComputePoseParam_LeanAmount(pStudioHdr);
	}
...
bool CHL2MPPlayerAnimState::HandleProning(Activity &idealActivity)
{
...
void CHL2MPPlayerAnimState::ComputePoseParam_MoveYaw( CStudioHdr *pStudioHdr )
{
	...
	// Leaning uses imovex just for fun
	/*if (m_pPlayer->m_Local.m_bLeaningLeft || m_pPlayer->m_Local.m_bLeanedLeft)
	{
		GetBasePlayer()->SetPoseParameter(pStudioHdr, m_PoseParameterData.m_iMoveX, 1.0f);
	}
	else if (m_pPlayer->m_Local.m_bLeaningRight || m_pPlayer->m_Local.m_bLeanedRight)
	{
		GetBasePlayer()->SetPoseParameter(pStudioHdr, m_PoseParameterData.m_iMoveX, -1.0f);
	}
	else
...
void CHL2MPPlayerAnimState::ComputePoseParam_LeanAmount(CStudioHdr *pStudioHdr)
{
	if (m_pPlayer->m_Local.m_bLeaningLeft || m_pPlayer->m_Local.m_bLeanedLeft)
	{
		float duration = (gpGlobals->curtime - m_PoseParameterData.m_flLeanStartTime)*TIME_TO_LEAN_INV;
		duration = max(-1, -duration);
		GetBasePlayer()->SetPoseParameter(pStudioHdr, m_PoseParameterData.m_iLeanAmount, duration);
		m_PoseParameterData.m_flLeanEndTime = -gpGlobals->curtime;
	}
	else if (m_pPlayer->m_Local.m_bLeaningRight || m_pPlayer->m_Local.m_bLeanedRight)
	{
		float duration = (gpGlobals->curtime - m_PoseParameterData.m_flLeanStartTime)*TIME_TO_LEAN_INV;
	...
	{
		if (m_PoseParameterData.m_flLeanEndTime > 0)
		{
			float duration = (gpGlobals->curtime - m_PoseParameterData.m_flLeanEndTime)*TIME_TO_LEAN_INV;
			duration = max(0, 1 - duration);
			GetBasePlayer()->SetPoseParameter(pStudioHdr, m_PoseParameterData.m_iLeanAmount, duration);
			if (duration == 0)
			{
				m_PoseParameterData.m_flLeanEndTime = 0;
			}
			
		}
		else if (m_PoseParameterData.m_flLeanEndTime < 0)
		{
			float duration = (gpGlobals->curtime + m_PoseParameterData.m_flLeanEndTime)*TIME_TO_LEAN_INV;
			duration = min(0, -1 + duration);
			GetBasePlayer()->SetPoseParameter(pStudioHdr, m_PoseParameterData.m_iLeanAmount, duration);
			if (duration == 0)
			{
				m_PoseParameterData.m_flLeanEndTime = 0;
		...
		{
			GetBasePlayer()->SetPoseParameter(pStudioHdr, m_PoseParameterData.m_iLeanAmount, 0);
			
		}
		m_PoseParameterData.m_flLeanStartTime = gpGlobals->curtime;
	}
