...
//
LINK_ENTITY_TO_CLASS(wall_pane, CWallPane);
BEGIN_DATADESC(CWallPane)
...
// Input  : pOther - 
//-----------------------------------------------------------------------------
void CWallPane::PaneTouch(CBaseEntity *pOther)
{
	if (pOther &&
...
// Output  :
//------------------------------------------------------------------------------
CWallPane* CWallPane::CreateWindowPane(const Vector &vecOrigin, const QAngle &vecAngles)
{
	CWallPane *pGlass = (CWallPane*)CreateEntityByName("wall_pane");
	if (!pGlass)
	{
	...
	if (pGlass->edict())
	{
		...
		pGlass->Spawn();
		pGlass->SetTouch(&CWallPane::PaneTouch);
...
//	> CBreakableWall
//####################################################################################
LINK_ENTITY_TO_CLASS(func_breakable_wall, CBreakableWall);

BEGIN_DATADESC(CBreakableWall)
...
IMPLEMENT_SERVERCLASS_ST(CBreakableWall, DT_BreakableWall)
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBreakableWall::Precache(void)
{
	UTIL_PrecacheOther("wall_pane");
...
// Output  :
//------------------------------------------------------------------------------
void CBreakableWall::SurfaceTouch(CBaseEntity *pOther)
{
...
// Output  :
//------------------------------------------------------------------------------
int CBreakableWall::OnTakeDamage(const CTakeDamageInfo &info)
{
...
//------------------------------------------------------------------------------
void CBreakableWall::TraceAttack(const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr, CDmgAccumulator *pAccumulator)
{
	//=============================================================================
	// HPE_BEGIN:
	...
	if (info.GetDamageType() & (DMG_BULLET | DMG_CLUB))
	{
		...
		if (ShatterPane(nWidth, nHeight, vecDir * 500, ptr->endpos))
		{
			...
			te->DispatchEffect(filter, 0.0, data.m_vOrigin, "WoodImpact", data);
		}
...
//			vDir - 
//-----------------------------------------------------------------------------
void CBreakableWall::Die(CBaseEntity *pBreaker, const Vector &vAttackDir)
{
	if (m_bIsBroken)
		return;
	...
	SetTouch(&CBreakableWall::SurfaceTouch);
}
...
// Input  : Force vector.
//------------------------------------------------------------------------------
void CBreakableWall::InputShatter(inputdata_t &inputdata)
{
...
// Output  :
//------------------------------------------------------------------------------
void CBreakableWall::Event_Killed(CBaseEntity *pInflictor, CBaseEntity *pAttacker, float flDamage, int bitsDamageType)
{
	return;
}
...
// Output  :
//------------------------------------------------------------------------------
bool CBreakableWall::IsBroken(int nWidth, int nHeight)
{
...
//			support - 
//-----------------------------------------------------------------------------
void CBreakableWall::SetSupport(int w, int h, float support)
{
...
// Output  :
//------------------------------------------------------------------------------
float CBreakableWall::GetSupport(int nWidth, int nHeight)
{
...
// Output  :
//------------------------------------------------------------------------------
float CBreakableWall::RecalcSupport(int nWidth, int nHeight)
{
...
// Output  :
//------------------------------------------------------------------------------
void CBreakableWall::BreakThink(void)
{
...
// Output  :
//------------------------------------------------------------------------------
void CBreakableWall::PanePos(const Vector &vPos, float *flWidth, float *flHeight)
{
...
// Output  :
//------------------------------------------------------------------------------
void CBreakableWall::BreakAllPanes(void)
{
...
// Output  :
//------------------------------------------------------------------------------
void CBreakableWall::BreakPane(int nWidth, int nHeight)
{
	...
	SetThink(&CBreakableWall::BreakThink);
	SetNextThink(gpGlobals->curtime);
}
...
// Output  :
//------------------------------------------------------------------------------
void CBreakableWall::DropPane(int nWidth, int nHeight)
{
	// Check parameter range
	if (nWidth < 0 || nWidth >= m_nNumWide) return;
	if (nHeight < 0 || nHeight >= m_nNumHigh) return;
	...
	if (!IsBroken(nWidth, nHeight))
	{
		...
		CWallPane *pPane = CWallPane::CreateWindowPane(vBreakPos, vAngles);
		if (pPane)
		{
...
void CBreakableWall::CreateShards(const Vector &vBreakPos, const QAngle &vAngles,
...
// Output  :
//------------------------------------------------------------------------------
bool CBreakableWall::ShatterPane(int nWidth, int nHeight, const Vector &vForce, const Vector &vForcePos)
{
	// Check parameter range
...
// Output  :
//------------------------------------------------------------------------------
void CBreakableWall::Spawn(void)
{
	BaseClass::Spawn();
	SetCollisionGroup(COLLISION_GROUP_BREAKABLE_GLASS);
	m_bIsBroken = false;
...
void CBreakableWall::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent)
{
	if (!m_bIsBroken)
	{
		int damageType = 0;
