...
BEGIN_SHADER( MyShader, "Help for my shader." )
	...
	// ----------------------------------------------------------------------------
	BEGIN_SHADER_PARAMS
	SHADER_PARAM(MYCOLOR, SHADER_PARAM_TYPE_COLOR, "[1 0 0 1]", "What color to render with.")
	END_SHADER_PARAMS
	SHADER_INIT_PARAMS()
	{
	...
	SHADER_DRAW
	{
		...
		DYNAMIC_STATE
		{
			...
			pShaderAPI->SetStencilReferenceValue(0);
			pShaderAPI->SetStencilTestMask(0xFF);
			pShaderAPI->SetStencilCompareFunction(STENCILCOMPARISONFUNCTION_EQUAL);
			pShaderAPI->SetStencilPassOperation(STENCILOPERATION_ZERO);
			/*int w;
			int h;
			...
			int x_end = x_start + w;
			int y_end = y_start + h;
			pShaderAPI->ClearStencilBufferRectangle(x_start, y_start, x_end, y_end, 0xFF);*/
		...
		if (pShaderAPI)
		{
			...
			//int h;
			//pShaderAPI->GetBackBufferDimensions(w, h);
			//pShaderAPI->ClearStencilBufferRectangle(0, 0, w, h, 0x1);
